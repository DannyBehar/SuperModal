...
class StandardInteractionController: NSObject, InteractionControlling {
    ...
    private func disableOtherTouchesIfNeeded() {
...
private extension StandardInteractionController {
    func hasActiveTextInputFocus() -> Bool {
        guard let rootView = viewController?.view else { return false }
        guard let firstResponder = rootView.findFirstResponder() else { return false }
